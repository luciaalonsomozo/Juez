...
protected:
   ...
   struct TreeNode {
      ...
      Link iz, dr;
      int altura;
      TreeNode(T const& e, int tam = 1, Link i = nullptr, Link d = nullptr,
               int alt = 1) : elem(e), iz(i), dr(d), altura(alt), tam_i(tam) {}
   };
...
protected:
   ...
   bool inserta(T const& e, Link & a) {
      ...
      if (a == nullptr) { // se inserta el nuevo elemento e
         ...
         if (crece) {
            a->tam_i++;//a pasa a tener un hijo más en la izquierda
            reequilibraDer(a);
         }
      } else if (menor(a->elem, e)) {
   ...
   void reequilibraDer(Link & a) { //Tiene que averiguar si hace falra reequilibrar y que tipo
      if (altura(a->iz) - altura(a->dr) > 1) {
         if (altura(a->iz->dr) > altura(a->iz->iz))
            rotaIzqDer(a); //Rotacion doble
         else rotaDer(a); //Rotacion simple
      }
      else a->altura = std::max(altura(a->iz), altura(a->dr)) + 1;
   }
   ...
   //El enunciado nos afirma que podemos suponer que no va a haber borrado.
   ...
   T const& kesimoProtected(int k, Link & a) const{
        if(a == nullptr || k > nelems){
        ...
        if(k == a->tam_i){
            ...
            return kesimoProtected(k, a->iz);
        }
        else {
            return kesimoProtected(k - a->tam_i, a->dr);
        }
   }
...
/*
template <class T>
T Set<T>::const& kesimo(int k) const{
    
    if(k == this.tam_i){
        return this->elem;
    }
    else if(menor(k, this.tam_i)){
        return this->iz.kesimo(k);
    }
    else if (k == this.tam_i +  this->dr.tam_i){
       return this->dr.elem;
    }
    else {
        return this->dr.kesimo(k-this->tam_i);
    }
...
bool resuelveCaso() {
   ...
   for(int i = 0; i < M; i++){
        std::cin >> indice;
        try{
         int resultado = arbol.kesimo(indice);
         std::cout << resultado;
        }
